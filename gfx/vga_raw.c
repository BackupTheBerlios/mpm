...
PRIVATE struct mode_list_s {
    ...
    { VGA_640x480x16, 0xa0000 - FB_BASE,
      { { 0xE3 },
...
#endif
    { VGA_320x200x256, 0xa0000 - FB_BASE,
      { { 0x63 },
...
PRIVATE int set_mode(gfx_mode_t mode) {
    ...
    for (x=0; x<VGA_NUM_AC_REGS; x++) {
        sys_inb(VGA_INSTAT_READ, &lv);
        sys_outb(VGA_AC_INDEX, x);
        sys_outb(VGA_AC_WRITE, rm->ac[x]);
    }
